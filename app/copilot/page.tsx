...
import { CheckCircle2, AlertTriangle, Activity, Users, BarChart3, DollarSign, Target, Brain, Shield, Settings, Play, Pause, FileText, Calendar, LineChart, Upload, Download, Rocket, Building2, ClipboardList, Mail, Phone, Wallet, TrendingUp, BookOpen, Layers, Search, ChevronRight, ChevronLeft } from "lucide-react";
...
export type GovernanceAction = {
  ...
  entity: string;
  entityId: Id;
  requirement: "ROI hypothesis" | "QA checklist" | "Owner" | "Payback window" | "TRS Score lever";
    ...
    <div className="sticky top-0 z-10 border-b bg-white/70 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <Badge variant="secondary" className="rounded-full">Internal</Badge>
        </div>
        <div className="ml-auto flex w-full max-w-xl items-center gap-4">
          ...
          <Button variant="outline" className="gap-2"><Download className="h-4 w-4"/>Export Deck</Button>
        </div>
      </div>
    </div>
  );
...
function Sidebar({ active, onChange }: { active: ModuleKey; onChange: (m: ModuleKey) => void }) {
  return (
      ...
      <nav className="space-y-1">
        {modules.map(m => (
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition ${active===m.key ? "bg-neutral-900 text-white" : "hover:bg-neutral-100"}`}
          >
            <m.icon className="h-4 w-4"/>
            <span className="text-sm font-medium">{m.label}</span>
            {m.key === "deliverables" && (
              <Badge className="ml-auto" variant={active===m.key?"secondary":"outline"}>3</Badge>
            )}
          </button>
        ))}
      </nav>
...
function SwitchInline({ enabled=false }: { enabled?: boolean }) {
  ...
  return (
    <button onClick={() => setOn(v=>!v)} className={`relative h-6 w-11 rounded-full transition ${on?"bg-neutral-900":"bg-neutral-300"}`} aria-label="toggle">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        {kpis.map(({ label, value, icon: Icon }) => (
          <Card key={label} className="rounded-2xl">
            <CardHeader className="pb-2"><CardTitle className="text-sm text-neutral-600">{label}</CardTitle></CardHeader>
      ...
      <Card className="rounded-2xl">
        ...
        <CardContent className="space-y-2">
          {["Raise list price 8% within floor/ceiling","Freeze lowest-ROI channel","Expansion trigger for Tier A accounts","Dunning for failed payments 7d","Partner launch: Rev-share with MSP Guild"].map((p,i)=> (
            <div key={i} className="flex items-center justify-between rounded-xl border p-3">
              <div className="flex items-center gap-3"><ChevronRight className="h-4 w-4"/><span className="text-sm font-medium">{p}</span></div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <Card className="rounded-2xl">
          <CardHeader><CardTitle className="text-base">Risks & Blockers</CardTitle></CardHeader>
        ...
        <Card className="rounded-2xl">
          <CardHeader><CardTitle className="text-base">Cadence</CardTitle></CardHeader>
...
function RiskRow({ icon: Icon, text, status }:{ icon: any; text: string; status: string }){
  return (
    <div className="flex items-center justify-between rounded-xl border p-3">
      <div className="flex items-center gap-3"><Icon className="h-4 w-4"/><span>{text}</span></div>
...
function CadenceRow({ icon: Icon, label, items }:{ icon:any; label:string; items:string[] }){
  return (
    <div className="rounded-xl border p-3">
      <div className="mb-2 flex items-center gap-2 font-medium"><Icon className="h-4 w-4"/>{label}</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="inbox">
        ...
        <TabsContent value="inbox"><InboxToRevenue /></TabsContent>
        <TabsContent value="deals"><DealTable /></TabsContent>
        <TabsContent value="renewals"><RenewalTable /></TabsContent>
      </Tabs>
    </div>
  );
}
      ...
      <CardContent className="space-y-2 text-sm">
        {Array.from({length:4}).map((_,i)=> (
          <div key={i} className="flex items-center justify-between rounded-xl border p-3">
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" variant="outline">Draft Reply</Button>
            ...
            {Array.from({length:3}).map((_,i)=> (
              <div key={i} className="flex items-center justify-between rounded-xl border p-3">
                <div>
                  ...
                  <div className="text-neutral-600">Prob: {Math.round(20+Math.random()*60)}% • Source: Outbound</div>
                </div>
                <div className="flex items-center gap-2">
...
function RenewalTable(){
  return (
    <Card className="rounded-2xl">
      <CardHeader><CardTitle className="text-base">Next 90 Days</CardTitle></CardHeader>
      <CardContent className="grid grid-cols-1 gap-2 md:grid-cols-2">
        {Array.from({length:6}).map((_,i)=> (
          <div key={i} className="rounded-xl border p-3 text-sm">
            <div className="flex items-center justify-between"><span className="font-medium">Account {i+1}</span><Badge variant="outline">Risk: {(10+i)%3?"Low":"High"}</Badge></div>
            <div className="text-neutral-600">Reason: Low adoption • Play: QBR + expansion offer</div>
            <div className="mt-2 flex items-center gap-2">
              <Button size="sm" variant="outline">Rescue Plan</Button>
        ...
        {agents.map(a => (
          <Card key={a.name} className="rounded-2xl">
            <CardHeader className="flex-row items-center justify-between">
              <div className="flex items-center gap-2"><a.icon className="h-4 w-4"/><CardTitle className="text-base">{a.name}</CardTitle></div>
              <Badge variant="secondary">KPI: {a.kpi}</Badge>
            </CardHeader>
            <CardContent className="flex items-center justify-between">
              <div className="text-sm text-neutral-600">Status: {a.status}</div>
              <div className="flex items-center gap-2">
                <Button size="sm" variant="outline"><Settings className="h-4 w-4"/>Config</Button>
                <Button size="sm"><Play className="h-4 w-4"/>Run</Button>
              </div>
            </CardContent>
          </Card>
        ))}
...
function QRASection(){
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Prioritized Plays</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {plays.map(p=> (
            <div key={p.title} className="flex items-center justify-between rounded-xl border p-3">
              <div>
                <div className="flex items-center gap-2"><Badge variant="secondary">{p.tier}</Badge><span className="font-medium">{p.title}</span></div>
                <div className="text-sm text-neutral-600">Forecast Uplift {p.uplift} • Time-to-Impact {p.tti}</div>
              </div>
              <div className="flex items-center gap-2">
...
function DealDesk(){
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Value-Metric Simulator</CardTitle></CardHeader>
        <CardContent className="flex items-end gap-3">
          ...
          <Button className="gap-2"><LineChart className="h-4 w-4"/>Simulate</Button>
        </CardContent>
      </Card>
    </div>
  );
...
function FPnA(){
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card className="rounded-2xl"><CardHeader><CardTitle className="text-base">Runway</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">18 months</div></CardContent></Card>
        <Card className="rounded-2xl"><CardHeader><CardTitle className="text-base">Free Cash Flow</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">$85k/mo</div></CardContent></Card>
        <Card className="rounded-2xl"><CardHeader><CardTitle className="text-base">Scenario</CardTitle></CardHeader><CardContent className="flex items-center gap-2"><Button variant="outline">Worst</Button><Button variant="secondary">Base</Button><Button variant="outline">Good</Button></CardContent></Card>
      </div>
    </div>
  );
}
...
function Retention(){
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">At-Risk Accounts</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {Array.from({length:4}).map((_,i)=> (
            <div key={i} className="rounded-xl border p-3 text-sm">
              <div className="flex items-center justify-between"><span className="font-medium">Account {i+1}</span><Badge variant="outline">Health: {(60+i)}%</Badge></div>
              <div className="text-neutral-600">Low adoption • Recommend: playbook + QBR</div>
              <div className="mt-2 flex items-center gap-2"><Button size="sm" variant="outline">Playbook</Button><Button size="sm">Trigger</Button></div>
            </div>
          ))}
        </CardContent>
      </Card>
...
function Experiments(){
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Active Tests</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          {["Pricing A/B","Onboarding Variant","Channel Budget Realloc"].map(name => (
            <div key={name} className="flex items-center justify-between rounded-xl border p-3">
              ...
              <div className="flex items-center gap-2"><Badge variant="secondary">Running</Badge><Button size="sm" variant="outline">Readout</Button></div>
            </div>
          ))}
        </CardContent>
      </Card>
...
function PartnerScout(){
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Scorecards</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2 text-sm">
          {Array.from({length:4}).map((_,i)=> (
            <div key={i} className="rounded-xl border p-3">
              <div className="flex items-center justify-between"><span className="font-medium">Partner {i+1}</span><Badge variant="outline">Overlap: {70+i}%</Badge></div>
              <div className="text-neutral-600">Deal influence high • Expected pipeline: ${50+i*10}k</div>
              <div className="mt-2 flex items-center gap-2"><Button size="sm" variant="outline">Offer</Button><Button size="sm">Activate</Button></div>
            </div>
          ))}
        </CardContent>
      </Card>
...
function ContentEngine(){
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Topic Map</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {["Pricing strategy","NRR mechanics","FP&A for founders","RevenueOS case"].map(t => (
            <div key={t} className="rounded-xl border p-3 text-sm">
              ...
              <div className="mt-2 flex items-center gap-2"><Button size="sm" variant="outline">Draft</Button><Button size="sm">Publish</Button></div>
            </div>
          ))}
        </CardContent>
      </Card>
...
function BillingRevOps(){
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Revenue Leakage</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          {["Failed payment recovery","Quote → Contract → Invoice sync","DSO coaching"].map(t => (
            <div key={t} className="flex items-center justify-between rounded-xl border p-3">
              <div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4"/><span className="font-medium">{t}</span></div>
...
function DeliverablesPanel(){
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {["Clarity Audit","Gap Map","Intervention Blueprint","RevBoard Dashboard","Monthly ROI Report","Quarterly ROI Synthesis","Case Study Packet"].map((d)=> (
          <Card key={d} className="rounded-2xl">
            <CardHeader><CardTitle className="text-base">{d}</CardTitle></CardHeader>
            <CardContent className="flex items-center justify-between text-sm">
              <Badge variant="secondary">In progress</Badge>
              <div className="flex items-center gap-2"><Button size="sm" variant="outline"><Upload className="h-4 w-4"/>Attach</Button><Button size="sm"><Download className="h-4 w-4"/>Export</Button></div>
            </CardContent>
          </Card>
        ))}
      </div>
...
function GovernancePanel(){
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Activation Gate</CardTitle></CardHeader>
        <CardContent className="space-y-2 text-sm">
          {items.map((it)=> (
            <div key={it.requirement} className="flex items-center justify-between rounded-xl border p-3">
              ...
              {it.status === "met" ? <Badge className="gap-1" variant="secondary"><CheckCircle2 className="h-3 w-3"/>Met</Badge> : <Badge variant="outline">Open</Badge>}
            </div>
          ))}
          <div className="pt-2"><Button className="gap-2" disabled><Play className="h-4 w-4"/>Enable Module</Button></div>
        </CardContent>
      </Card>
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">AI Engine Compliance</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2 text-sm">
          <div className="rounded-xl border p-3">
            <div className="mb-1 font-medium">Model: QRA-Forecast v1.2</div>
            <div>F1 0.78 • MAPE 9% • Thresholds: F1≥0.75, MAPE≤10%</div>
            <div className="mt-2 flex items-center gap-2"><Badge variant="secondary">Approved</Badge><Button size="sm" variant="outline">View Card</Button></div>
          </div>
          <div className="rounded-xl border p-3">
...
function IntegrationRow({ name, status }:{ name:string; status:"Connected"|"Pending" }){
  return (
    <div className="flex items-center justify-between rounded-xl border p-3">
      <div className="font-medium">{name}</div>
