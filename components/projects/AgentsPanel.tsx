...
export function AgentsPanel() {
    ...
    startTransition(async () => {
      try {
        ...
        if (!response.ok) {
          ...
          setStatus(payload?.error ? `Failed: ${payload.error}` : "Agent run failed");
          return;
        }
        setStatus(`Agent run logged: ${kind}`);
      } catch (error) {
        setStatus("Unable to reach agent service");
      }
    });
  }, []);
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-3">
      ...
      {status && <div className="mt-3 text-xs text-gray-600">{status}</div>}
    </div>
  );
}
